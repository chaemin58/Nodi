...
export function CurrentNodi({
  ...
  const [isOpenCreateMeetup, setIsOpenCreateMeetup] = useState<boolean>(false);
  return (
    <>
      {isOpenCreateMeetup && (
        <CreateMeetupModal onClose={() => setIsOpenCreateMeetup(false)} groupId={groupId} />
      )}
      <div className="flex flex-col gap-3 lg:gap-5">
        ...
        <Button size="md" onClick={() => setIsOpenCreateMeetup(true)}>
          약속 추가하기
        </Button>
      </div>
    </>
